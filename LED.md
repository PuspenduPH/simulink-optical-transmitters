...
The subsystem is masked, prompting the user for the physical parameters. Because the inputs for the LED (wavelength, lifetimes) are requested in nanometers and nanoseconds for user convenience, the background script first converts these inputs back to standard SI units (meters, seconds) before calculation.
